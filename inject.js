...
$(elements).each((index, element) => {
  ...
  chrome.storage.sync.get([id], function (result) {
    ...
    if (isListened)
      log(() => `Album: ${albumlink} - IsListened: ${isListened}`);
    const isListenedSelectedClass = isListened
      ? selectedClass
      : unselectedClass;
    ...
    const toggleButton = $(
      `<div class="albuminContainer"><i class="albuminIcon ${albumClass} ${isListenedSelectedClass} fas fa-compact-disc"></i><i class="albuminIcon ${eggClass} ${isEggedSelectedClass} fa-egg fas"></i></div>`
    );
...
function extractAlbumName(element) {
  ...
  if (isList) {
    const artistAlbumTitle = $element
      .siblings('.albumListTitle')
      .find('a')
      .text();
...
function onToggleSaveAlbumClick(event) {
  ...
  chrome.storage.sync.set(albumDescriptors, function () {
    ...
    $target
      ...
      .toggleClass(`${selectedClass} ${unselectedClass}`);
  });
}
